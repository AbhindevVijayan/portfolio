...
# CORS - allow the MCP server (running locally) to call the internal API if needed
CORS_ALLOWED_ORIGINS = os.getenv("CORS_ALLOWED_ORIGINS", "http://localhost:8000").split(",")

# Shared secret the MCP server uses to authenticate against admin-only API endpoints
